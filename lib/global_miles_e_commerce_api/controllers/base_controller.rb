  ...
  class BaseController
    ...
    def validate_response(context)
      if context.response.status_code == 400
        raise APIException.new('Bad Request	- The request was unacceptable, ' \
                               'often due to missing a required parameter.',
                               context)
      elsif context.response.status_code == 401
        raise APIException.new('Unauthorized - No valid API key provided.',
                               context)
      elsif context.response.status_code == 402
        raise APIException.new('Request Failed - The parameters were valid b' \
                               'ut the request failed.',
                               context)
      elsif context.response.status_code == 404
        raise APIException.new('Not Found - The requested resource doesn't e' \
                               'xist.',
                               context)
      elsif context.response.status_code == 429
        raise APIException.new('Too Many Requests - Too many requests hit th' \
                               'e API too quickly.',
                               context)
      elsif context.response.status_code == 500
        raise APIException.new('Server Error - Something went wrong.',
                               context)
      end
